...
@Service
@Slf4j
public class CompanyScholarshipSuccessfulCandidateProcessor {
	...
	public CompanyScholarshipGenericDto addUpdateCompanyScholarshipSuccessfulCandidate(String userId , String companyId, String companyScholarshipId, CompanyScholarshipGenericDto companyScholarshipGenericDto) throws NotFoundException, UnauthorizeException, BadRequestException {
		...
		log.info("Adding Successful Candidate description for company scholarship id {}",companyScholarshipId);
		companyScholarship.setSuccessfulCandidateRequirment(companyScholarshipGenericDto.getDescription());
	...
	@Transactional
	public CompanyScholarshipGenericDto getCompanyScholarshipSuccessfulCandidate(String userId , String companyId ,String companyScholarshipId) throws NotFoundException {
		log.debug("Inside CompanyScholarshipSuccessfulCandidateProcessor.addUpdateCompanyScholarshipEntranceRequirement() method");
		log.info("Getting company with companyId {}", companyId);
		Optional<Company> optionalCompany = companyDao.getCompanyById(companyId);
		...
		log.info("Generating CompanyScholarshipGenericDto wit Successful Candidate descriptionfrom model");
		return new CompanyScholarshipGenericDto(companyScholarship.getSuccessfulCandidateRequirment());
	}
